...
# Ensure you have Python installed (default for Ubuntu) and the PIL and lxml
# packages:
# pip install -f requirements.txt or sudo apt-get install python-imaging
# python-lxml
#
#
# RUN AT STARTUP
...
#
# DEFAULTS
# While the script will detect as much as possible and has safe defaults, you
# may want to set your own.
#
# DOWNLOAD_PATH  - where you want the file to be downloaded. Will be auto-
# detected if not set.
# CUSTOM_FOLDER  - if we detect your download folder, this will be the target
# folder in there.
# RESOUTION_TYPE -
#     'stretch': single monitor or the combined resolution of your available
# monitors
#     'largest': largest resolution of your available monitors
#     'default': use the default resolution that is set
# RESOLUTION_X   - horizontal resolution if RESOLUTION_TYPE is not default or
# cannot be
#                  automatically determined
# RESOLUTION_Y   - vertical resolution if RESOLUTION_TYPE is not default or
# cannot be
#                  automatically determined
# NASA_APOD_SITE - location of the current picture of the day
# IMAGE_SCROLL   - if true, will write also write an XML file to make the
# images scroll
# IMAGE_DURATION - if IMAGE_SCROLL is enabled, this is the duration each will
# stay in seconds
# SEED_IMAGES    - if > 0, it will download previous images as well to seed the
# list of images
...
# Use XRandR to grab the desktop resolution. If the scaling method is set to
# 'largest',
# we will attempt to grab it from the largest connected device. If the scaling
# method is set to 'stretch' we will grab it from the current value. Default
# will simply use what was set for the default resolutions.


def find_resolution():
    if RESOLUTION_TYPE == 'default':
        if SHOW_DEBUG:
            print "Using default resolution of %sx%s" % (
...
def set_download_folder():
    if os.access(DOWNLOAD_PATH, os.W_OK):
        ...
        raise ValueError('Download path %s is not accessible'
                         % (DOWNLOAD_PATH))
...
def create_desktop_background_scoll(filename):
    ...
    if num_images < SEED_IMAGES:
        ...
        while seed_images_left > 0:
            ...
            # If the content was an video or some other error occurred,
            # skip the rest.
    ...
    # Get our images in a random order so we get a new order every time
    # we get a new file
    ...
    for i, image in enumerate(images):
        ...
        # Check to see if we're at the end, if we are use the first
        # image as the image to
        if i + 1 == num_images:
